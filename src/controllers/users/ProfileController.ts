...
export class ProfileController {
  ...
  static async listPostsByUsername(req: AuthedRequest, res: Response): Promise<Response> {
    try {
      ...
      const before =
        typeof beforeRaw === "string" && beforeRaw ? new Date(beforeRaw) : undefined;
      if (before && Number.isNaN(before.getTime())) {
        return fail(res, 400, "POSTS_VALIDATION", "Invalid before cursor.");
      }
      const posts = await PostService.listByUsername(
        req.userId!,
